...
# 2. 配置参数
# ==========================================
LOG_DIR = "./logs_ablation_doorkey"  # 必须与训练脚本中的 LOG_DIR 一致
        ...
        for file in files:
            if file.endswith("monitor.csv"):
                ...
                if "Baseline" in folder_name:
                    algo = "Baseline"
                    # Baseline 实际上就是 Sparse Reward，但在图例里简写为 Baseline 即可
                ...
                try:
                    ...
                    # 平滑曲线 (Window=200, 适合 200k 步的数据)
                    df['Smoothed_Reward'] = df['Reward'].rolling(window=200, min_periods=10).mean()
...
def plot_ablation():
    ...
    # 颜色方案
    PALETTE = {
        "Ours (Hierarchical Potential)": "#c0392b",       # 深红色 (Highlight)
        "Simple Shaping (Goal Dist)": "#f39c12",          # 橙黄色 (Warning/Mid)
        "Baseline": "#34495e"                             # 深灰色 (Background)
    }
    ...
    sns.lineplot(
        ...
        dashes=DASHES,
        errorbar='sd',  # 显示标准差阴影
        linewidth=2.0,       # 线条稍微细一点
        alpha=0.9,           # 线条透明度
        errorbar='sd', # [技巧] 只显示 0.5 倍标准差，或者干脆用 'ci', 95
        ax=ax
    )
    
    # 坐标轴设置
    ax.set_ylim(-0.05, 1.05) # 限制在 0-1 之间
